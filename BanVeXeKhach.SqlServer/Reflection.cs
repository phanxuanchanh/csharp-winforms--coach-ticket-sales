...
namespace BanVeXeKhach.SqlServer
{
    ...
    public class Reflection
    {
        ...
        public FieldInfo GetField(object obj, Func<FieldInfo, bool> predicate)
        {
            return GetFields(obj).SingleOrDefault(predicate);
        }

        public FieldInfo GetField(object obj, string fieldName)
        {
            return GetField(obj, f => f.Name == fieldName);
        }
        ...
        public object SetValuesForFieldsOfObject(object model, Dictionary<string, object> pairs)
        {
            Type type = model.GetType();
            if (type.IsValueType || type.Name == "String")
                return null;
            Dictionary<string, object> fields = pairs.Where(f => Regex.IsMatch(f.Key, "(^.)[a-zA-Z0-9]{1,}$"))
                .ToDictionary(f => f.Key, f => f.Value);
            foreach (KeyValuePair<string, object> field in fields)
            {
                FieldInfo fieldInfo = GetField(model, field.Key);
                if (fieldInfo != null)
                    fieldInfo.SetValue(model, field.Value);
            }
            return model;
        }
    }
